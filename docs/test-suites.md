...
| 5 - Wischsemantik | sie **fährt die Geste** und liest, welches Reveal-Panel aufgeht: eine Liste mit Wischzeilen antwortet überhaupt, und die Rolle liegt an ihrer Kante (`--delete` nie am Zeilenanfang, `--done` nie am Ende). Der Finger geht vor dem Loslassen unter die Schwelle zurück, damit die Sonde nichts abhakt und nichts löscht | 16 Routen + jeder Budget-Untertab × mobil × `de`/`ar` |
| 6 - Kennzahlreihen | die Kacheln **einer** `.metric-grid` sind gleich hoch, auch wenn die Reihe umbricht: die Höhe gehört dem Träger, nicht dem längsten Text einer Zelle | 16 Routen + jeder Budget-Untertab × mobil/desktop |
...
**Warum Sonde 5 und 6 hinter die Untertabs klicken.** Von den sieben Kennzahlreihen der App liegt genau eine auf einer eigenen Route; die übrigen hängen an den Budget-Untertabs, die nach der Leisten-Regel eine SICHT innerhalb eines Moduls wechseln und deshalb keine Route bewegen. Dasselbe gilt für die Abo-Wischliste. Eine Sonde, die nur Routen abfährt, sähe eine Kennzahlreihe von sieben - und ausgerechnet die Abo-Reihe, die den Befund trug, wäre unsichtbar geblieben. Beide Sonden nehmen deshalb denselben Helfer (`visitBudgetTabs`) und suchen ihren Gegenstand auf jeder Route **und** hinter jedem Tab, statt eine Fundortliste zu führen: so finden sie auch, was es heute noch nicht gibt. Der Preis ist Laufzeit (rund drei Minuten für Sonde 6, eine für Sonde 5), und er ist bewusst gezahlt.
