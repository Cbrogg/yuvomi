...
**Warum Sonde 6 hinter die Untertabs klickt.** Von den sieben Kennzahlreihen der App liegt genau eine auf einer eigenen Route; die übrigen hängen an den Budget-Untertabs, die nach der Leisten-Regel eine SICHT innerhalb eines Moduls wechseln und deshalb keine Route bewegen. Eine Sonde, die nur Routen abfährt, sähe eine von sieben - und ausgerechnet die Abo-Reihe, die den Befund trug, wäre unsichtbar geblieben. Sie sucht darum jede `.metric-grid` auf jeder Route **und** hinter jedem Tab, statt eine Fundortliste zu führen: so findet sie auch eine Kennzahlreihe, die es heute noch nicht gibt. Der Preis ist Laufzeit (rund drei Minuten), und er ist bewusst gezahlt.
