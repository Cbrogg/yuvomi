...
| target_outlook_account_id | INTEGER | Outlook account ID for the one-way push (migration v131), nullable. Mutually exclusive with the other target columns |
| target_outlook_calendar_id | TEXT | Microsoft Graph calendar ID for the one-way push (migration v131), nullable |
...
(migration v131). Multi-account like CalDAV Accounts; OAuth tokens live per account row, **not** in
...
| content_hash | TEXT | SHA-256 of the last pushed payload — unchanged events are no-ops (0 requests) |
