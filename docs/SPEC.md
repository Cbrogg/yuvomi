...
**Recurring series and the two ID spaces (v1.52.1 · #593):** the inbound list runs with `singleEvents: true`, so Google returns a series as separate *instances* (`<masterId>_<timestamp>`), each without a `recurrence` field — which is why a series imported from Google is stored as individual events rather than a master with an RRULE. A series *uploaded* by Yuvomi is the opposite: `events.insert` returns the **master** id, and the local row keeps its RRULE and is expanded locally. Both together used to duplicate every occurrence — the locally expanded master plus Google's instances of the same series. The inbound now skips an instance whose `recurringEventId` matches a local row that already holds the master and an RRULE, and clears instance rows left over from before the fix, but only those the user never touched (`user_modified = 0`, no assignments); anything recoloured or assigned stays and has to be removed by hand. Consequences worth knowing: for an uploaded series, moving it and changing its repeat rule work, because the row carries the master id. For a series imported *from* Google both are rejected with `400`, since they address a single instance. And an exception created in Google on a series that Yuvomi uploaded does not arrive, because its instance is skipped along with the rest.
