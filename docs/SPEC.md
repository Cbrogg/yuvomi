...
Normalization is shared by every entry point (`server/utils/task-tags.js`): trimmed, empties dropped, capped at 32 tags of 64 characters each, and case-folded so `Garten` and `garten` are one tag (first spelling wins). A value that breaks a limit is truncated rather than rejected — a sync run must not fail on one over-long foreign tag.

- **Filtering:** `GET /api/v1/tasks?tag=…` accepts the parameter repeatedly (`?tag=a&tag=b`) or comma-separated. Several tags **narrow** the list (a task must carry all of them), matching how the other filters in the same bar compose. Case-insensitive. Only the repeated form survives a tag that itself contains a comma.
