...
- Advanced settings — Single Sign-On (OIDC), automatic backups, off-site WebDAV backups, local-folder/WebDAV/Google Drive document storage, the host data folder (`DATA_DIR`), the calendar sync interval and the private-network switches
...
> `2.14.3` (exact version), `2.14` (latest patch of that minor), plus a moving `main`
> tag for the current development state. To pin production to a known-good release,
> set `image: ghcr.io/ulsklyc/yuvomi:2.14.3` in your compose file and bump it
...
```
yuvomi  | [Yuvomi] Server running on port 3000 | Version 2.14.3
