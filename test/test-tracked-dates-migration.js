/**
 * Test: reminders-Tabellen-Rebuild fuer inventory_tracked_date (Migration v138)
...
const V138 = MIGRATIONS.find((m) => m.version === 138);

function seedPreV138() {
...
test('v138 ist als foreignKeysOff-Migration deklariert', () => {
  assert.equal(V138.foreignKeysOff, true);
});

test('v138 erhält bestehende Erinnerungen und Zustellprotokolle', () => {
  const db = seedPreV138();
...
test("v138 erlaubt entity_type 'inventory_tracked_date'", () => {
  const db = seedPreV138();
  db.pragma('foreign_keys = OFF');
  db.exec(V138.up);
  db.pragma('foreign_keys = ON');
...
test('v138 legt inventory_item_dates mit FK auf inventory_items und Updated-At-Trigger an', () => {
  const db = seedPreV138();
  db.pragma('foreign_keys = OFF');
  db.exec(V138.up);
  db.pragma('foreign_keys = ON');
...
test('Gegenbeweis: mit aktiver FK-Durchsetzung würde der DROP die Zustellprotokolle mitreißen', () => {
  ...
  assert.equal(db.prepare('SELECT COUNT(*) AS c FROM notification_deliveries').get().c, 0,
    'belegt, warum foreignKeysOff an v138 nicht wegfallen darf');
  db.close();
});
