...
- **Modules can be switched on and off from a phone again** (audit of #673). The new admin page inherited a rule written for the two locked rows - Overview and Settings, whose switch does nothing anyway - because it reuses the same three-column row. Below 640px that hid all eleven switches, on the one page that can turn a module off and on the device this household uses most. The rule now names what it always meant.
- **The household switch and the personal one no longer share a row** (design critique of #673). Turning a module off for everyone and hiding it for yourself sat twelve pixels apart, both unlabelled, and the one that takes a module from six people without confirmation was the louder of the two. Switching a module on or off for the household has moved to its own admin page, **Settings → Modules → Active modules**; Settings → Personal → Navigation is now entirely yours - order, mobile favourites, and what you want to see. The eye button keeps one icon and one name that says which module it means, states its reason when a module is off for the household, and no longer throws away keyboard focus when you hide several modules in a row. Hiding a module that held one of your three mobile favourites now says which position changed instead of letting you find it on the phone later. A hidden row is tinted rather than dimmed, which brings its icon back over the contrast minimum.
- **Every member can take a module out of their own navigation** (#673). Switching a module off was possible, but only for an admin and only for the whole household, so a member who never opens Housekeeping had it in the sidebar for good. Each row in Settings → Personal → Navigation now carries an eye button that hides that module for the person clicking it - the household switch beside it is unchanged and still admin-only. Hiding is tidying, not a withdrawal: a link from a notification, a dashboard widget or the search still opens the page, and what someone may not reach stays a matter of permissions. Hidden modules also stop being offered as mobile favourites, and Kitchen can be hidden as a group or one station at a time. The page description said "order and mobile navigation" and now says what the page has actually been able to do.
- **Customizing the overview says whose board it is while you are changing it** (#585). The line that others keep their own arrangement arrived in the confirmation after saving; the doubt sits before it. It now stands in the customize bar. "Reset" also says what it resets - since the arrangement became personal, the word had two plausible meanings and delivered a third.
- **Every member arranges their own overview** (#585). Which tiles the board shows, in what order and at what size, was stored once for the whole household: whoever took the cycle tile off their board took it off everyone's, and whoever pulled tasks to the top moved them for the children too. A family has different needs on the same page, which is what the request said. The arrangement and the "Today at a glance" band now belong to the member who set them, stored the same way the navigation order and the calendar defaults already were. Nothing changes on upgrade: as long as nobody rearranges anything, everyone keeps seeing the household's saved board, and the first personal change only affects that one person. The saved-confirmation says so now, in place of a message that named the page by a word the app does not otherwise use. On a shared wall tablet the loading skeleton no longer predicts the previous member's grid after a change of user.
