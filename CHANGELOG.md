...
- **An optional extension API proxy** forwards `/api/v1/extensions/:moduleId/*` to sidecar upstreams configured via `EXTENSION_PROXY_TARGETS` on the Yuvomi container (with SSRF-hardened outbound fetch).
...
- **OpenAPI now documents extension module capabilities**, module i18n metadata, and the optional extension proxy route.
