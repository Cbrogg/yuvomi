...
- Amounts typed the way the app itself suggested were rejected on saving. A shared expense showed "0,00" as its hint, and in Persian or Egyptian Arabic even "۰٫۰۰", but only a dot-separated ASCII number ever reached the server. Entering the suggested format now works in every language. Amounts written with thousands separators are still refused rather than reinterpreted - in German a dot groups thousands, yet "12.50" almost always means twelve-fifty, and silently dropping it would turn the amount into 1250.
