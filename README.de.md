...
Achtzehn eigenständige Module teilen sich eine ruhige, konsistente Oberfläche. Schalte ein, was dein Haushalt braucht; der Rest bleibt aus dem Weg.
...
| ![recipes](https://raw.githubusercontent.com/ulsklyc/yuvomi/main/docs/icons/recipes.png) | **Rezepte** | Rezepte erstellen, duplizieren und skalieren, dann Mahlzeiten-Slots vorbefüllen, die Zutaten direkt auf eine Einkaufsliste schicken oder jede geplante Mahlzeit speichern. Spiegelt auf Wunsch eine selbst gehostete Mealie- oder Tandoor-Instanz, nur lesend. |
...
| ![inventory](https://raw.githubusercontent.com/ulsklyc/yuvomi/main/docs/icons/inventory.png) | **Inventar** | Was du besitzt: Marke, Modell, Seriennummer, Lagerort, Kaufpreis, Garantie und Zustand, mit verknüpften Belegen und Buchungen und Erinnerungen, bevor eine Frist abläuft. |
...
- **Rezepte** - Rezepte erstellen, duplizieren und skalieren; Mahlzeiten-Slots vorbefüllen, die Zutaten direkt auf eine Einkaufsliste schicken oder jede geplante Mahlzeit als Rezept speichern. Spiegelt auf Wunsch eine selbst gehostete Mealie- oder Tandoor-Instanz: deren Rezepte erscheinen neben deinen eigenen mit Quellen-Badge und Rücklink, du planst und kaufst mit ihnen ein wie mit jedem anderen Rezept, und der gespiegelte Server bleibt die Quelle der Wahrheit für ihren Inhalt - dupliziere eines, um eine bearbeitbare Kopie zu bekommen.
...
- **Inventar** - Was du besitzt: Marke, Modell, Seriennummer, Lagerort, Kaufpreis und -datum, Garantiedauer, Zustand und Status, dazu ein optionales Foto. Zweistufig durchstöbern - erst Kennzahlkarten und eine Kategorieübersicht, dann die Gegenstände einer Kategorie nach Lagerort gruppiert. Belege und Anleitungen aus den Dokumenten verknüpfen, dazu Kauf, Erstattung, Reparatur oder nachträgliches Zubehör aus dem Budget - ein Gegenstand direkt aus einer Buchung anzulegen füllt seinen Preis automatisch vor. Eine abgeleitete Garantiefrist erinnert 30 Tage vor Ablauf, und bis zu zehn frei definierbare Fristen je Gegenstand (TÜV, Service, Versicherung) teilen sich dieselben Erinnerungen und ein abonnierbares ICS-Feed.
...
- **Erinnerungen** - Erinnerungen zu Aufgaben und Terminen per In-App-Badge, Opt-in-Web-Push (HTTPS) und Haushalts-Kanälen über Gotify, ntfy oder generische HTTP-Webhooks. Ein Webhook sendet JSON an einen beliebigen Endpunkt, mit optionalem, nur schreibbarem Bearer-Token und einer optionalen Payload-Vorlage für Dienste, die ihr eigenes Format erwarten, etwa Discord oder Slack; Details stehen im [Webhook-Leitfaden](docs/notification-webhooks.md).
