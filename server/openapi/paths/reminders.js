...
export function remindersPaths() {
  return {
    ...
    '/api/v1/reminders': {
      ...
      delete: op({ summary: 'Delete reminders by filter', tag: 'Reminders', stateChanging: true, description: 'Derived types (`subscription`, `inventory_item`, `inventory_tracked_date`, `pantry_item`) are rejected with 400 - their module recreates the row. Dismiss them instead.' }),
    },
    '/api/v1/reminders/{id}/dismiss': {
      patch: op({ summary: 'Dismiss reminder', tag: 'Reminders', params: [idParam()], stateChanging: true }),
    },
    '/api/v1/reminders/{id}': {
      delete: op({ summary: 'Delete reminder', tag: 'Reminders', params: [idParam()], stateChanging: true, description: 'Derived types are rejected with 400 - their module recreates the row. Dismiss them instead.' }),
    },
  };
}
