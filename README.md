...
| | Module | In one line |
|:---:|---|---|
| ![tasks](https://raw.githubusercontent.com/ulsklyc/yuvomi/main/docs/icons/tasks.png) | **Tasks** | Kanban board with deadlines, priorities, subtasks, recurring schedules and multi-member assignment. |
...
| ![calendar](https://raw.githubusercontent.com/ulsklyc/yuvomi/main/docs/icons/calendar.png) | **Calendar** | Google and CalDAV sync, ICS subscriptions, recurring events, holiday overlays and shared visibility. |
...
| ![rewards](https://raw.githubusercontent.com/ulsklyc/yuvomi/main/docs/icons/rewards.png) | **Rewards** | Points on tasks credit assigned members, with a parent-approved catalog and an auditable ledger. |
...
- **Budget** - Income, expenses, recurring entries, trend charts, a statistics tab, CSV export, accounts with starting balances and running totals plus net worth, loans (optionally as an annuity with a fixed, variable or fixed-then-variable interest rate, with the payment, term and total interest derived live), split expenses, subscription tracking with renewals and currencies, a planning tab with per-category monthly budgets and a savings goal (planned vs. actual), and an optional personal budget mode where each entry can be private or shared with a My budget / Household view.
...
- **Notes &amp; Contacts** - Colored Markdown sticky notes that open in a rendered reader view (toggle to edit), with full-text search, a per-author filter and pinned notes grouped up front, plus a contact directory with CardDAV sync and multi-contact vCard import/export.
